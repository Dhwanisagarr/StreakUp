...
function ChallengeForm({ initial, onSave, onClose }) {
  ...
  const handleSave = () => {
    ...
    onSave({ name: name.trim(), description: desc.trim(), startDate: start, duration: dur, habits: habits.map(h => h.name) });
  };
  ...
  return (
    <Sheet onClose={onClose} maxHeight="96vh">
      <div style={{ padding: "20px 20px 0" }}>
        ...
        <button onClick={handleSave} style={{ ...solidBtn, width: "100%", padding: "16px", marginTop: 16, marginBottom: 8, fontSize: 16, fontWeight: 800 }}>
  ...
  return (
    <div style={{ paddingBottom: 100 }}>
      ...
      {activeChallenges.map(c => {
        ...
        const allChecked = c.habits.every(h => habitChecks[h]);
                ...
                <>
                  ...
                  {checkedCount > 0 && !allChecked && (
                    <p style={{ margin: "0 0 14px", fontSize: 13, color: C.textMuted }}>
                      {checkedCount}/{c.habits.length} habits checked — check all to complete today.
                    </p>
                  )}
                  ...
                  <button
                    ...
                    style={{
                      ...
                      marginBottom: 10,
                      background: allChecked ? `linear-gradient(135deg, ${C.amber}, ${C.amberDark})` : C.amber,
                      boxShadow: allChecked ? "0 4px 20px rgba(245,158,11,0.4)" : "none",
                      transform: "translateY(0)",
                      transition: "all 0.2s ease",
                    }}
                    onMouseEnter={e => allChecked && (e.currentTarget.style.transform = "translateY(-1px)")}
                    onMouseLeave={e => (e.currentTarget.style.transform = "translateY(0)")}
                  >
                    {allChecked ? "✓ Complete Today" : `Check all ${c.habits.length} habits to complete`}
                  </button>
...
───────────────────────────────────────────── */
function DetailScreen({ challenge, records, onBack, onEdit, onDuplicate, onDelete }) {
  ...
  return (
    <div style={{ paddingBottom: 100 }} onClick={() => showMenu && setShowMenu(false)}>
      ...
      <div style={{ margin: "0 16px 20px", background: C.amberLight, borderRadius: 24, border: `1.5px solid ${C.amberBorder}`, padding: "20px 20px 18px" }}>
        ...
        <p style={{ margin: "0 0 16px", fontSize: 13, color: C.textMuted }}>{fmtDate(challenge.startDate)} → {fmtDate(end)} · {challenge.duration} days</p>
      ...
      <div style={{ margin: "0 16px 20px", background: C.card, borderRadius: 20, border: `1px solid ${C.border}`, padding: "18px 18px" }}>
        <h3 style={{ margin: "0 0 14px", fontSize: 15, fontWeight: 800, color: C.text }}>Daily Habits ({challenge.habits.length})</h3>
      ...
      <div style={{ margin: "0 16px 20px", background: C.card, borderRadius: 20, border: `1px solid ${C.border}`, padding: "18px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(7,1fr)", gap: 4 }}>
          ...
          {Array.from({ length: daysInMonth }).map((_, i) => {
            ...
            return (
              <div key={day} style={{ height: 32, borderRadius: 8, background: bg, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, fontWeight: inRange ? 600 : 400, color: bg === C.green ? "#fff" : C.textMid, border: ds === td ? `2px solid ${C.amber}` : "none" }}>
                {day}
              </div>
            );
          })}
        ...
        <div style={{ display: "flex", gap: 12, marginTop: 12, flexWrap: "wrap" }}>
          {[[C.green, "Completed"], [C.amberMid, "Skipped"], [C.redLight, "Missed"], [C.border, "Future"]].map(([bg, label]) => (
...
export default function StreakUp() {
  ...
  const [tab, setTab] = useState("today"); // today | challenges | stats
  ...
  const completeDay = useCallback((challengeId) => {
    const td = todayStr();
    setAllRecords(prev => ({
      ...prev,
